...
def test_stage2(opt, Mine_model_init, Mine_model_His, Mine_model_Cls,Mine_model_molecular, Mine_model_Graph, dataloader, gpuID,external=False,flag_tiantan=False):
    ...
    return list_Diag,list_Diag_GBM,list_Diag_G4A,list_Diag_G3A,list_Diag_G2A,list_Diag_G3O,list_Diag_G2O, \
           list_DiagSim, list_DiagSim_GBM, list_DiagSim_G4A, list_DiagSim_G23A, list_DiagSim_G23O,list_IDH,list_1p19q,list_CDKN

def test_allmarker(opt, Mine_model_init, Mine_model_molecular, Mine_model_Graph, dataloader):
    Mine_model_init.eval()
    Mine_model_molecular.eval()
    Mine_model_Graph.eval()
    gpuID = opt['gpus']
...
def test_stage1_stem(opt,Mine_model_init,Mine_model_His,Mine_model_Cls,dataloader, gpuID,epoch):
    metrics = ['acc', 'sen', 'spec', 'precision', 'recall', 'f1', 'AUC']
    ...
    if epoch < 7:
        dataframe.to_excel('./logs/' + opt['name'] + '/result.xlsx', sheet_name='epoch' + str(epoch),
                           index=False)
    else:
        with pd.ExcelWriter('./logs/' + opt['name'] + '/result.xlsx', mode='a', engine='openpyxl') as writer:
            dataframe.to_excel(writer, sheet_name='epoch' + str(epoch), index=False)



def test_pre_stem(opt,Mine_model_init,Mine_model_His,Mine_model_Cls, trainLoader,dataloader, gpuID,epoch):
    metrics = ['acc', 'sen', 'spec', 'precision', 'recall', 'f1', 'AUC']
    list_his, list_A, list_O, list_GBM, list_grade, list_G2,\
    ...
    if epoch < 7:
        dataframe.to_excel('./logs/' + opt['name'] + '/Pre-result.xlsx', sheet_name='epoch' + str(epoch), index=False)
    else:
        with pd.ExcelWriter('./logs/' + opt['name'] + '/Pre-result.xlsx', mode='a', engine='openpyxl') as writer:
            dataframe.to_excel(writer, sheet_name='epoch' + str(epoch), index=False)


def test_stage2_stem(opt,Mine_model_init,Mine_model_body,Mine_model_Cls,Mine_model_molecular, Mine_model_Graph, dataloader, gpuID,epoch):
    metrics = ['acc', 'sen', 'spec', 'precision', 'recall', 'f1', 'AUC']
    list_all_Diag, list_GBM, list_G4A, list_G3A, list_G2A, list_G3O, list_G2O,\
    list_all_DiagSim, list_GBMSim, list_G4ASim,list_G23ASim,list_G23OSim,list_IDH,list_1p19q,list_CDKN = test_stage2(opt, Mine_model_init, Mine_model_body, Mine_model_Cls,Mine_model_molecular, Mine_model_Graph, dataloader, gpuID)
    ...
    if epoch < 7:
        dataframe.to_excel('./logs/' + opt['name'] + '/result.xlsx', sheet_name='epoch' + str(epoch),index=False)
    else:
        with pd.ExcelWriter('./logs/' + opt['name'] + '/result.xlsx', mode='a', engine='openpyxl') as writer:
            dataframe.to_excel(writer, sheet_name='epoch' + str(epoch), index=False)
...
def test_surv(opt, Mine_model_init, Mine_model_His, Mine_model_Cls,Mine_model_molecular, Mine_model_Graph,Mine_model_surv,testLoader_all, gpuID,epoch):
    ...
    for packs in test_bar:
        ...
        IDH = label[:, 6].to(torch.float32)
        pq = label[:, 7].to(torch.float32)
        CDKN = label[:, 8].to(torch.float32)
        subtype = label[:, 9].to(torch.float32)
        grade = label[:, 10].to(torch.float32)
        count += 1
        saliency_map_His, saliency_map_Grade = saliency_map_read_stage2(opt, file_name)
        saliency_map_His = torch.from_numpy(np.array(saliency_map_His)).float().cuda(gpuID[0])
        saliency_map_Grade = torch.from_numpy(np.array(saliency_map_Grade)).float().cuda(gpuID[0])
        ...
        ### ### surv pred
        hazards, S,logits = Mine_model_surv(encoded_Grade, encoded_IDH,age,sex,IDH,pq,CDKN,subtype,grade)
        ...
        label_all.append(label_surv.cpu().numpy()[0])
        predicted_all.append(logits.detach().cpu().numpy()[0][1])
    ...
    with open('./logs/' + opt['name'] + '/cindex.txt', 'a+') as f:
        f.write('epoch: %d cindex: %.3f auc: %.3f' % (epoch, c_index,auc))
        f.write('\n')
    print('testLoader: Epoch: {}, c_index: {:.4f}, auc: {:.4f}'.format(epoch, c_index,auc))


def test_predall(opt,Mine_model_init,Mine_model_molecular, Mine_model_Graph, dataloader, gpuID,epoch,testDataset_IvYGAP):
    metrics = ['acc', 'sen', 'spec', 'precision', 'recall', 'f1', 'AUC']
    list_marker= test_allmarker(opt, Mine_model_init, Mine_model_molecular, Mine_model_Graph, dataloader)
    print('test in epoch: %d/%d, '% (epoch, 70)+ opt['marker']+' || acc:%.3f,sen:%.3f,spec:%.3f, auc:%.3f' % ( list_marker[0], list_marker[1], list_marker[2], list_marker[6]))

    dataframe = pd.DataFrame({'metrics': metrics,'marker': list_marker})
    if epoch < 7:
    ...
    ######### external
    ######### IvYGAP
    if opt['marker']=='MGMT' or opt['marker']=='EGFR' or opt['marker']=='PTEN':
        list_marker= test_allmarker(opt, Mine_model_init, Mine_model_molecular, Mine_model_Graph, testDataset_IvYGAP)
        print('test in epoch: %d/%d, '% (epoch, 70)+ opt['marker']+' || acc:%.3f,sen:%.3f,spec:%.3f, auc:%.3f' % (list_marker[0], list_marker[1], list_marker[2], list_marker[6]))
